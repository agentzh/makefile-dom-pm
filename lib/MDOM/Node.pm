...
The C<MDOM::Node> class provides an abstract base class for the Element
classes that are able to contain other elements L<MDOM::Document>,
L<MDOM::Statement>, and L<MDOM::Structure>.

As well as those listed below, all of the methods that apply to
L<MDOM::Element> objects also apply to C<MDOM::Node> objects.
...
### XS -> MDOM/XS.xs:_MDOM_Node__scope 0.903+
sub scope { '' }
...
The C<add_element> method adds a L<MDOM::Element> object to the end of a
C<MDOM::Node>. Because Elements maintain links to their parent, an
...
the C<MDOM::Node> object. Note that in the base of the L<MDOM::Structure>
classes, this C<DOES> include the brace tokens at either end of the
structure.

Returns a list of zero or more L<MDOM::Element> objects.
...
the C<MDOM::Node> object. As for the C<elements> method, this does include
the brace tokens for L<MDOM::Structure> objects.

Returns a L<MDOM::Element> object, or C<undef> if for some reason the
C<MDOM::Node> object does not contain any elements.
...
C<MDOM::Node> object. Note that in the case of the L<MDOM::Structure>
classes, this does B<NOT> include the brace tokens at either end of the
structure.

Returns a list of zero of more L<MDOM::Element> objects.
...
The C<child> method accesses a child L<MDOM::Element> object by its
position within the Node.

Returns a L<MDOM::Element> object, or C<undef> if there is no child
...
such as whitespace and comments, while L<MDOM> treats these items as valid
...
The C<contains> method is used to determine if another L<MDOM::Element>
object is logically "within" a C<MDOM::Node>. For the special case of the
brace tokens at either side of a L<MDOM::Structure> object, they are
generally considered "within" a L<MDOM::Structure> object, even if they are
not actually in the elements for the L<MDOM::Structure>.

Returns true if the L<MDOM::Element> is within us, false if not, or C<undef>
on error.
...
L<MDOM::Element> objects that meet a particular condition.
...
The function will be passed two arguments, the top-level C<MDOM::Node>
you are searching in and the current L<MDOM::Element> that the condition
is testing.
...
The C<find> method returns a reference to an array of L<MDOM::Element>
...
sub find {
	...
	eval {
		while ( my $Element = shift @queue ) {
			...
			next unless $Element->isa('MDOM::Node');
...
Returns the first L<MDOM::Element> object that matches the condition, false
...
If passed a L<MDOM::Element> object that is a direct child of the Node,
...
sub remove_child {
	my $self  = shift;
	my $child = _INSTANCE(shift, 'MDOM::Element') or return undef;
...
The C<prune> method is used to strip L<MDOM::Element> objects out of a code
...
sub _wanted {
	...
	$it = "MDOM::$it" unless substr($it, 0, 6) eq 'MDOM::';
	unless ( _CLASS($it) and $it->isa('MDOM::Element') ) {
		# We got something, but it isn't an element
		Carp::carp("Cannot create search condition for '$it': Not a MDOM::Element") if $^W;
		return undef;
	}
...
####################################################################
# MDOM::Element overloaded methods
...
sub __link_children {
	...
	while ( my $Node = shift @queue ) {
		...
		foreach my $Element ( @{$Node->{children}} ) {
			...
			unshift @queue, $Element if $Element->isa('MDOM::Node');
		}

		# If it's a structure, relink the open/close braces
		next unless $Node->isa('MDOM::Structure');
		Scalar::Util::weaken(
...
- Move as much as possible to L<MDOM::XS>
